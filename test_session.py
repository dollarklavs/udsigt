...
with Session(gisdb="/tmp/grassdb_test", location="test",
   ...
   # do something in permanent
    ...
    gcore.run_command('v.to.rast', input='barriere', output='barriere_raster',use = 'attr', attribute_column = 'hoejde')   
    gcore.run_command('r.mapcalc',expression = 'tempdsm_m_barriere = if(isnull(barriere),tempdsm)')
    #grass.mapcalc('tempdsm_m_barriere = if(isnull(barriere),tempdsm)')
    gcore.run_command('r.viewshed', input='tempdsm_m_barriere', output='viewshed', max_distance=1000, memory=1424, coordinates=(701495,6201503), observer_elevation=500.0)
    gcore.run_command('r.out.gdal', overwrite=True, input='viewshed', output=output_tif)
# {u'GISDBASE': u"'/tmp/';",
