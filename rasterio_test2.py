...
out_rst = '/tmp/dsm/shp_rasterized.tif'
    ...
    with rasterio.Env():
        result = features.rasterize(
				    ((feature['geometry'],np.int(feature['properties']['hoejde'])) 
                                    ...
                                    , out_shape=src_rst.shape,transform=src_rst.transform,all_touched=True)
        with rasterio.open(out_rst,'w',**out_meta ) as out:
            out.write(result.astype(rasterio.int16), indexes=1)
